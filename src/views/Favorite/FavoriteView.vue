    ...
    <h1 class="text-white flex justify-center text-2xl md:text-3xl">Minha Lista de Favoritos</h1>
      <div>
        <template v-if="favoriteList.length === 0">
          <p class="text-center text-base padding md:text-xl text-gray-500 pt-4"> Hum... parece que você ainda não tem favoritos.
        ...
        <template v-else>
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4">
            <template v-for="item in favoriteList" :key="item.id">
              <movies-card v-if="isMovie(item)" :movie="item" @favorite-removed="handleFavoriteRemoved"
              />
              <series-card v-else :series="item" @favorite-removed="handleFavoriteRemoved"
              />
            </template>
        </div>
        </template>
...
import { defineComponent } from 'vue';
  ...
  export default defineComponent({
    ...
    methods:{
      ...
      handleFavoriteRemoved(itemId: number) {
