<template>
    ...
    <section v-else class="relative h-screen overflow-hidden bg-fixed bg-cover">
        ...
        <img :src="backgroundImage" :alt="series.original_name"
            ...
            <div class="bg-opacity-80 flex top-24 md:top-64 xl:top-52 z-30 relative items-center p-10 flex-col lg:flex-row-reverse gap-8 md:flex md:gap-12 lg:gap-0 justify-center lg:justify-between">
                ...
                <div v-else :class="{ 'hidden': !isNotFoundVideoLoaded }"
                    ...
                    <img :src="backgroundImage" alt="Image Not Found" @load="onNotFoundVideoLoad"
                        class="rounded-xl w-48 h-48 md:h-72 object-cover opacity-75 md:w-96" />
                </div>
                <card-on class="w-full sm:w-5/6 md:w-5/6 lg:w-2/5 p-4 rounded-lg shadow-lg bg-black bg-opacity-50">
                    <template #title> 
                    ...
                    <template #footer>
                        <div class="flex justify-between gap-8 md:grid md:grid-cols-3">
                            <button-on @click="navigateBack" label="Voltar" icon="pi pi-arrow-left"
                                class="rounded-lg font-bold py-2 text-white px-4 md:pr-24 pr-8 hover:bg-white hover:text-black" />
                            <button-on @click="toggleFavorite" :label="favoriteLabel" :icon="favoriteIcon" :class="{
                                ...
                                'border-white text-white px-5  md:px-16': !isFavorite,
    ...
    methods: {
        ...
        setTrailerUrl() {
            if (this.series.videos && this.series.videos.results) {
                for (const video of this.series.videos.results) {
                    if (video.type === 'Trailer' && video.site === 'YouTube') {
                        this.trailerUrl = `https://www.youtube.com/embed/${video.key}?controls=0`;
                        break;
                    }
                }
            }
